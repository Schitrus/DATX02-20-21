...
void main() {
   ...
   for(float t = 0.0; t<=D; t+=h){
           ...
           float lum = clamp((texture(temperature, tr).x-50.0)/50.0, 0.0, 1.0);
           float rad = clamp((texture(temperature, tr).x-100.0)/300.0, 0.0, 1.0);
           float hot = clamp((texture(temperature, tr).x-400.0)/800.0, 0.0, 1.0);
           //calculate Alpha

           //accumulating collor and alpha using under operator
           vec3 baseColor = mix(mix(mix(vec3(0.05,0.05,0.05),
