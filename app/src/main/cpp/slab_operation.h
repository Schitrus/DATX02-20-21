...
public:
    ...
    void buoyancy(DataTexturePair* velocity, DataTexturePair* temperature, vec3 size, float dt, float scale);
    // Performs advection on the given data
    void advection(DataTexturePair* velocity, DataTexturePair* data, vec3 lowerResolution, vec3 higherResolution, float dt);

    void fulladvection(DataTexturePair* velocity, vec3 lowerResolution, vec3 higherResolution, DataTexturePair* data, float dt);

    void temperatureOperation(DataTexturePair* temperature, DataTexturePair* velocity, vec3 lowerResolution, vec3 higherResolution, float dt);

    // Adds the given source field multiplied by dt to the target field
    void addSource(DataTexturePair* data, vec3 resolution, GLuint& source, float dt);
    void setSource(DataTexturePair* data, vec3 resolution, GLuint& source, float dt);

    void dissipate(DataTexturePair* data, vec3 size, float dissipationRate, float dt);

    //example values: iterationCount = 20, diffusionConstant = 1.0
    void diffuse(DataTexturePair* data, vec3 size, int iterationCount, float diffusionConstant, float dt);

    // Projects the given *vector* field
    void projection(DataTexturePair* velocity, vec3 size);
...
private:
    ...
    void jacobi(vec3 size, int iterationCount);

    // Calculates the divergence of the vector field
    void createDivergence(DataTexturePair* vectorData, vec3 size);

    // Subtracts the gradient of the given scalar field from the target vector field
    void subtractGradient(DataTexturePair* velocity, vec3 size);

    void setBoundary(DataTexturePair* data, vec3 size, int scale);
    ...
    void interiorOperation(Shader shader, vec3 size, DataTexturePair* data);

    // Performs the operation with the set shader over the entirety of the given data.
    // You must set the shader program, along with any uniform input or textures needed by the shader beforehand.
    void fullOperation(Shader shader, vec3 size, DataTexturePair* data);
    ...
    void drawFrontOrBackBoundary(DataTexturePair* data, vec3 size, int scale, int depth);
    ...
    void drawAllToTexture(Shader shader, vec3 size, int depth);

    // Sets the depth uniform on the shader and then draws the interior
    // Should be called after the relevant call to prepareResult()
    void drawInteriorToTexture(Shader shader, vec3 size, int depth);

    // Sets the depth uniform on the shader and then draws the boundary
    // Should be called after the relevant call to prepareResult()
    void drawBoundaryToTexture(Shader shader, vec3 size, int depth);
};
